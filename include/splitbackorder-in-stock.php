...
/*
  Slipt Back Order
 * */
   class sbow_split_according_class {
        ...
        public function init() {
            if (!is_admin() || (defined('DOING_AJAX') && DOING_AJAX)) {
                $this->sbow_frontend_hooks();
            }
        }

        public function sbow_frontend_hooks() {
            add_action('woocommerce_checkout_order_processed', array($this, 'sbow_change_total_on_checking'), 20, 1);
            add_filter('woocommerce_thankyou_order_received_text', array($this, 'sbow_sunarc_change_order_received_text'), 10, 2);
				add_filter('woocommerce_locate_template', array($this, 'sbow_sunarc_thank_you_page_template'), 10, 3);
			}


        public function sbow_change_total_on_checking($order_id) {
            ...
            foreach ($items as $item => $valData) {
				...
				$product = wc_get_product($valData['product_id']);
			    $productids = $product->id; 
				$stockquantity = get_post_meta($productids, '_stock', true );
				$valData['quantity'];
		        if ($stockquantity > $valData['quantity']) {
                        $address = array(
...
//                 create sub order //
				...
				$sum = $valData['line_total'];
                $_product = $valData['data']->post;
                $product_title = $_product->post_title;
                $qty = $valData['quantity'];
                $price = get_post_meta($valData['product_id'], '_price', true);
                $variation_id = $valData['variation_id'];
                ...
                if ($item_id) {
                    ...
                    wc_add_order_item_meta($item_id, '_tax_class', $valData['tax_class']);
                    wc_add_order_item_meta($item_id, '_product_id', $valData['product_id']);
                    wc_add_order_item_meta($item_id, '_variation_id', $variation_id);
                    wc_add_order_item_meta($item_id, '_line_subtotal', $valData['line_subtotal']);
                    wc_add_order_item_meta($item_id, '_line_subtotal_tax', $valData['line_subtotal_tax']);
                    wc_add_order_item_meta($item_id, '_line_total', $valData['line_total']);
                    wc_add_order_item_meta($item_id, '_line_tax', $valData['line_tax']);
                    wc_add_order_item_meta($item_id, '_line_tax_data', array('total' => array($valData['total']), 'subtotal' => array($values['subtotal'])));
                    if ($valData['variation_data'] && is_array($item['variation_data'])) {
                ...
                $orderIds[] = $order->id;
					...
					{
					   $arrValuesout['quantity'][] = $valData;		
					}
                } 
		   foreach ($arrValuesout as $val) {
                $address = array(
                    'first_name' => sanitize_text_field($_POST['billing_first_name']),
                    'last_name' => sanitize_text_field($_POST['billing_last_name']),
                    'company' => sanitize_text_field($_POST['billing_company']),
...
//                 create sub order //
        ...
        function sbow_sunarc_change_order_received_text($str, $order) {
            $order_id = method_exists($order, 'get_id') ? $order->get_id() : $order->id;
            $str .= $this->sbow_change_order_received_text($order_id);
            return $str;
        }

        function sbow_change_order_received_text($order_id, $deleted = true) {
			include('mail.php');
        ...
        public function sbow_change_formatted_meta_data($orders) {
            $orders = array(
                'limit' => -1,
                'offset' => null,
                'page' => 1,
		...
		public function sbow_sunarc_thank_you_page_template($template, $template_name, $template_path) {
            if ('checkout/thankyou.php' == $template_name) {
                $template = sbow_sunarc_plugin_dir . '/include/checkout/thankyou.php';
            }
            return $template;
...
new sbow_split_according_class();
